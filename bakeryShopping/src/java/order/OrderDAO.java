        ...
        try {
            ...
            if (con != null) {
                if (status.equals("completed")) {
                    sql = "UPDATE [Order] "
                            + "SET [Order].SaleEmail = ? , [Order].Status = ? , [Order].CompleteDate = ?, [Order].Paid = 1 "
                            + "WHERE [Order].Id = ? ";
 ...
 /*----------------------------------------------------------------------------*/
    public boolean createOrder(OrderDTO order, List<OrderItemDTO> items, String address, float shipPrice, boolean paid) {
        ...
        try {
            ...
            String sql = "INSERT INTO [Order](CustomerEmail, Status, SaleEmail,CompleteDate,TotalOrderPrice, Address, ShippingPrice, Paid) VALUES (?,?,?,?,?,?,?,?)";
